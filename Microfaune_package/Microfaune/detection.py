...
class RNNDetector:
    ...
    #Function added to get scores from downsampled Audiomoth data
#    def predict_on_numpy_array(self, data):
#        data = scipy.signal.resample(data,int(len(data)*44100/384000))
#        X = self.compute_features([data])
#        scores, local_scores = self.predict(np.array(X))
#        return scores[0], local_scores[0]
